...
def selected_session_api(request):
    """Create a builder session while honoring the studio explicitly chosen by the user."""
    ...
    for category in wanted:
        candidates = api_v2.available_candidates(qs, category, start, duration, genres)
        if candidates:
    ...
    with transaction.atomic():
        chosen = [studio] + team
        locked = {x.id: x for x in Listing.objects.select_for_update().filter(id__in=[x.id for x in chosen], active=True)}
        ...
        SessionTask.objects.bulk_create([
            ...
            SessionTask(session=session, title="Setup vorbereiten", assignee_name=selected_team[-1].name if selected_team else "Team", due_label="Session", order=3),
        ])
